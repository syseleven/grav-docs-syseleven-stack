...
You can use this template as usual, only that you reference the public SSH Key you stored in the Dashboard using the command line switch `--parameter key_name=<PUB_KEY_NAME>`. This ensures that you can log in to the default account on your virtual machine using SSH.
...
You can also build a setup where a virtual machine using a shared network storage flavor has more than 50 GiB of storage. In that case you do not need to create the volume in a separate stack, you can just expand the orchestration template for your virtual machine.
...
In this case a clean solution is to delete the dependencies by hand - for example first delete the floating IP that is attached to the router, then delete the router and then the whole stack. Oftentimes you can also just call `openstack stack delete <STACK_NAME>` multiple times.  
Again, by specifying `depends_on: <MY_OTHER_RESOURCE_ID>` you can avoid this class of problem entirely.

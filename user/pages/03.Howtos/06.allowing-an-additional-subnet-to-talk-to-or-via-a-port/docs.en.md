...
```shell
openstack port set --allowed-address ip-address=<IP_ADDRESS_OR_IP_SUBNET_MASK> <PORT_ID>
```
...
Note that `openstack port set` returns no output. You can verify the change with `openstack port show <PORT_ID>`.
